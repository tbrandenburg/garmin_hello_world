...
if [ -n "${GARMIN_USERNAME:-}" ] && [ -n "${GARMIN_PASSWORD:-}" ]; then
    ...
    echo "      ERROR: GARMIN_USERNAME and GARMIN_PASSWORD environment variables must be set"
    echo "      These are required for SDK download in headless environments"
    echo ""
    echo "      For CI/CD, set these as secrets:"
